(() => {
  const leadsConnectMultipleForms = (pagesAndForms) => {
      ...
      popup.innerHTML = `
        <div class="popup-fancybox">
          <div class="popup-wrapper">
            <iframe name="form-leads_connect" id="form-leads_connect" src="${iframe}" frameborder="0" width="100%" height="1100" allowfullscreen=""></iframe>
    ...
    const dataForms = pagesAndForms
  
    const formsController = {
      getData(dataset, value) {
        return dataset.find(form => form.page.toLowerCase().replace(' ', '').replace('-', '').replace('/', '') === value.toLowerCase().replace(' ', '').replace('-', '').replace('/', ''))
      },
    ...
    const hasPage = page => dataForms.filter(data => data.page.toLowerCase().replace(' ', '').replace('-', '').replace('/', '') === page.toLowerCase().replace(' ', '').replace('-', '').replace('/', '')).length > 0
  ...
  leadsConnectMultipleForms([ { page: '/novos', form: 'https://hondabrasil.secure.force.com/CapturaLeads/myHonda_DynamicLeadCaptureForm?id=MjAyMDA2MTYxNDE5NTR8fDAwMTYxMDAwMDBOck9OSkFBMw==' }, { page: '/seminovos', form: 'https://hondabrasil.secure.force.com/CapturaLeads/myHonda_DynamicLeadCaptureForm?id=MjAyMDA2MTYxNDE5NTR8fDAwMTYxMDAwMDBOck9OSkFBMw==' } ])
})()